...
calculateSimilarity counts (x:xs) =
  (x * Map.findWithDefault 0 x counts) + calculateSimilarity counts xs

calculateSimilarityFromString :: String -> Int
calculateSimilarityFromString input =
  let lists = parseLists input
